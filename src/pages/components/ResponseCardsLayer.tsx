...
export function ResponseCardsLayer({
  ...
  return (
    <>
      {cards.map((card) => {
        const isLongAnswer = card.text.length > 64;
        ...
        return (
          ...
          >
            <div className={`${styles.cardShell} ${card.collapsed ? styles.collapsedShell : styles.expandedShell}`}>
              <div className={`${styles.cardFrame} ${card.pinned ? styles.pinned : ""}`}>
                <div className={styles.cardToolbar}>
                  <button
                    className={styles.cardButton}
                    onClick={() => onToggleCollapsed(card.id)}
                    title={card.collapsed ? "Expandir respuesta completa" : "Contraer respuesta"}
                    type="button"
                  >
                ...
                {card.collapsed ? (
                  ...
                  <div className={styles.plainAnswer}>{card.text}</div>
                ) : (
                  <div className={`latex-content ${styles.latexAnswer}`}>{card.latex}</div>
                )}
              </div>
            </div>
          </Draggable>
